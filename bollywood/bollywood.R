...
# Store the movie name in variable named top_2_movie
  ...
  top_2_movie <-   bollywood[which(bollywood$Tcollection==sort(bollywood$Tcollection,decreasing = TRUE)[2]),"Movie"]
...
# Store the names of 4 movies in same sequence in movie_result vector
  ...
  movie_result <- c(bollywood[which.max(bollywood$Ocollection),"Movie"],bollywood[which.max(bollywood$Wcollection),"Movie"],
                  bollywood[which.max(bollywood$Fwcollection),"Movie"],bollywood[which.max(bollywood$Tcollection),"Movie"])
